...
struct Node *createnode(int8_t data)
{
    ...
    if (newnode == NULL)
    {                                            // check si une addresse mémoire a été alloué avec notre fonction createnode, si pas d'allocation -> NULL
...
{ // insert node à appeler dans le main createnode
    if (root == NULL)
    {                            // si le noeud racine est vide
        return createnode(data); // on crée le noeud de manière classique avec createnode en associant data au nouveau noeud
    }

    if (data < root->data)
    {                                              // si data est inférieur à la valeur du noeud racine on place la valeur à gauche (sous arbre gauche)
...
// save
void saveTreeToFile(struct Node *root, FILE *file)
{
    if (root == NULL)
    {
        fprintf(file, "# ");
...
// load
struct Node *loadTreeFromFile(FILE *file)
{
    char buffer[10];
    if (!fscanf(file, "%s", buffer))
    {
        printf("Fin de fichier ou lecture impossible.\n");
        return NULL;
    }

    printf("Lecture : %s\n", buffer); // Débogage pour chaque élément lu

    if (strcmp(buffer, "#") == 0)
    {
        return NULL; // Si le marqueur "#" est rencontré, retourne NULL
    }

    int data = atoi(buffer);
    printf("Création d'un noeud avec la valeur : %d\n", data); // Débogage pour les nœuds
...
{ // pointeur vers node, noeud root valeur à suppr
    if (root == NULL)
        return root; // si l'arbre est vide, peut rien faire
    if (key < root->data)
    {                                             // si valeur entrée plus petite que root on attribu à data et on check
    ...
    {
        if (root->left == NULL)
        {                                    // si le noeud n'a pas d'enfant a gauche
        ...
        { // on fait l'inverse pour la gauche
...
{ // pas de retour, paramètre pointeur vers le noeud
    if (node == NULL)
        return;                  // si le noeud est NULL, pas de printtraversal fin de la branche de l'arbre
...
{                   // ne renvoie pas de valeur, pointeur comme d'hab vers le noeud root, int nombre d'espace pour créer l'arbre, on va voir ce que ça donne
    int count = 10; // indente de 10 espace à chaque fois
    if (root == NULL)
    { // si le noeud et vide, return, cut.
        return;
    }
    ...
    {                // Plus on est loin dans le noeud plus y'aura d'espece
